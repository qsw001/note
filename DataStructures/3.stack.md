# 栈

## 1.1 栈的定义和基本运算

栈（stack）：一种受限定的线性表，限定其插入和删除操作只能在表一端进行，其中允许进行操作的这一端叫做栈顶，另一端叫做栈底，栈中插入操作叫进栈或入栈，删除操作叫做出栈或退栈。

栈的特点：后进先出LIFO(Last In Fist Out)。

栈的基本运算：
- 初始化栈
- 判断栈是否为空
- 入栈
- 出栈
- 读取栈顶元素

## 1.2 顺序栈

顺序栈(Sequential Stack)是用数组来实现栈结构的一种方式

### 1.2.1 顺序栈的基本结构

```
#define MAX 1000

typedef struct {
    char data[MAX];  // 存放栈内数据
    int top;         // 栈顶指针
} Stack;
```

### 1.2.2 顺序栈的运算实现

**初始化栈**
```
void initStack(Stack* s) {
	s->top = -1;
}
```
**判断栈是否为空**

```
int isEmpty(Stack s) {
	if (s.top == -1) {
		return 1;
	}
	return 0;
}
```

**入栈**
```
int push(Stack* s, int x) {
	if (s->top == MAX - 1) {
		printf("栈满，入栈失败");
		return 0;
	}
	s->data[++s->top] = x;
	return 1;
}
```

**出栈**
```
int pop(Stack* s, int* x) {
	if (s->top == -1) {
		printf("栈空，出栈失败");
		return 0;
	}
	*x = s->data[s->top--];
	return 1;
}
```

**读取栈顶元素**
```
int getTop(Stack* s, int* x) {
	if (s->top == -1) {
		printf("栈空，读取失败");
		return 0;
	}
	*x = s->data[s->top];
	return 1;
}
```

## 1.3 链栈

链栈是通过链式存储结构实现的栈

### 链栈的基本结构

```
typedef struct node {
	struct node* next;
	int data;
}StackNode;
```

### 链栈的运算实现

**初始化栈**
```
void init(StackNode* top) {
	top->next = NULL;
}
```
**判断栈是否为空**

```
int isEmpty(StackNode* top) {
	if (top->next == NULL) {
		printf("栈为空\n");
		return 1;
	}
	printf("栈不为空\n");
	return 0;
}
```

**入栈**
```
void push(StackNode* top, int x) {
	StackNode* node = (StackNode*)malloc(sizeof(StackNode));
	node->data = x;
	node->next = top->next;
	top->next = node;
}
```

**出栈**
```
void pop(StackNode* top, int* x) {
	if (isEmpty(top)) {
		return;
	}
	StackNode* p = top->next;
	top->next = p->next;
	*x = p->data;
	free(p);
}
```

**读取栈顶元素**
```
void getTop(StackNode* top, int* x) {
	if (isEmpty(top)) {
		return;
	}
	*x = top->next->data;
}
```

## 1.4 栈的应用 

### 1.4.1 平衡符号

判断输入的符号，如(){}[]是否合法

平衡符号的思想：将输入左括号等入栈，然后对后续所输入的括号进行判断，如果可以与栈顶的括号匹配，则出栈，如果发生不能匹配的情况则说明匹配出错，当结束时，如果栈为空，则说明表达式合法，如果栈中还有剩余符号，则说明不合法。

代码位置: *Note/DataStructures/code/stack_BalanceSymbols*

平衡符号的应用：
- 编译器语法分析
- html的语法检验
- 计算式表达式的检验
  
### 1.4.2 中缀表达式转后缀表达式

中缀表达式(Infix Expression)：即为我们日常看到的表达式，运算符写在操作符之间，特点为人类看起来直观，但是计算机处理起来不容易，需要用栈进行辅助运算。

后缀表达式(Postfix Expression)：也叫逆波兰表达式，运算符写在操作符后面，特点为没有括号，不需要考虑优先级，计算机非常容易求值。

**中缀表达式转后缀表达式算法思路**：输入的表达式会有操作符，运算符，左右括号，所以我们对遇到上述情况分开讨论。

从左到右进行扫描。

- 操作符：遇到操作符时，我们直接写入新的表达式中。
- 左括号：遇到做左括号时，我们将其压入栈中。
- 右括号：遇到右括号时，我们将栈中的元素一一弹出，放入新的表达式中，直至遇到左括号。
- 运算符：若栈顶的元素的运算符优先级与它同级或更高，将栈顶元素弹出，并将它压入栈中；反之，直接将它压入栈中。

当扫描完成时，将栈中剩余元素弹入新的表达式中。

## 1.5栈易错和重点的知识点

栈为空时，top指向负一。

链栈使用的是头插法。

两个栈共享一个数组的优势是节省存储空间，降低上溢发生的概率

上溢：当栈满时，再进行push操作。

下溢：当栈空时，再进行pop操作

括号匹配用到栈的思想

栈的特点：
- 先进后出
- 只能在一端进行操作
- 操作受限