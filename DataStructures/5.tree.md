...
非空的二叉树的先序遍历与后序遍历的结果相反，则这个树一定只有一个叶子节点。
...
### 1.4.3 满二叉树与完全二叉树的易错点与重点

对于完全二叉树：其叶子节点的计算公式为 $⌈n / 2⌉$.
...
n个节点的线索二叉树共有：$n+1$个线索,因为n个节点的二叉树有n+1个空指针。

## 1.6 哈夫曼树与哈夫曼编码

编码：用某种规则把信息从某种形式转换为另一种形式的过程。
...
编码时我们必须要注意的是需要在不产生歧义的情况下尽可能的短小，由此产生出哈夫曼编码。

哈夫曼编码的构造过程如下所示：
将每一个字符出现的次数“可视化”，将出现最小的两个字符当作两个叶子节点，其和视为父亲节点，然后从小到大递归构造，直至结束，将左子树的路径赋值为0，右子树的路径赋值为1，这个树叫做哈夫曼树。结束后，从根节点到其叶子节点的路径编码顺序组合即为各个字符的编码，由于字符全为叶子节点，所以不可能有每个编码出现在另一个编码内。

### 1.6.1 哈夫曼树的易错点及难点

n个权值(叶子)构成的哈夫曼树共有$2n-1$个节点。

## 1.7 树与森林

### 1.7.1 树的四种存储方法

1. 双亲表示法：即每个节点存储一个指向双亲的下标。
2. 孩子表示法：每个节点存储指向所有孩子的指针。
3. 孩子链表表示法：每个节点带一个链表，链表里存储它所有的孩子。
4. 双亲孩子表示法：既存双亲，也存孩子。
5. 孩子兄弟表示法：孩子兄弟表示法中有两个指针，第一个指针指向左孩子，第二个指针指向右兄弟，此时，两个指针刚好可以变成二叉树的二叉链表存储，于是便可以将树转换为二叉树。

### 1.7.2 树的易错及重点
我们可以得出，树的先序遍历与其对应的二叉树的先根遍历所得的序列相同。

$$边数=节点数-树的棵树$$

要注意树和二叉树，二叉树和森林等之间的转换。

# 易错

对于二叉树，叶子节点数目比度数为二的节点数少1。
